...
$requests = [];
try {
    $stmt = $pdo->prepare("SELECT * FROM event_requests WHERE user_id = :user_id ORDER BY created_at DESC");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO event_requests (user_id, system_name, event_type, start_date, end_date, location, landmark, description, status) 
...
<head>
    ...
    <title>Public Reservation Request</title>
...
<body class="bg-lgu-bg min-h-screen p-4">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="header-gradient rounded-2xl shadow-lg p-8 mb-8 text-white flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold">Public Reservation Request</h1>
                    <p class="text-blue-100">Submit requests for public events</p>
                </div>
            </div>
        ...
        <div id="formModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-2xl font-bold text-lgu-headline">Event Details</h2>
                ...
                <form id="reservationForm" class="space-y-4">
                    <div>
                        ...
                        <input type="text" value="Public" readonly class="w-full border-2 border-gray-300 rounded-lg p-3 text-sm bg-gray-100 cursor-not-allowed">
                        <input type="hidden" name="system_name" value="Public">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-lgu-headline mb-2">Event Type <span class="text-lgu-tertiary">*</span></label>
                        <input type="text" name="event_type" placeholder="e.g., Concert, Sports Event" required class="form-input w-full border-2 border-gray-300 rounded-lg p-3 text-sm focus:outline-none focus:border-lgu-button">
                    </div>
                    ...
                    <div>
                        ...
                        <div id="reservationMap" class="w-full h-48 rounded-lg border-2 border-gray-300"></div>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-lgu-headline mb-2">Description <span class="text-lgu-tertiary">*</span></label>
                        <textarea name="description" placeholder="Describe your event..." required rows="3" class="form-input w-full border-2 border-gray-300 rounded-lg p-3 text-sm focus:outline-none focus:border-lgu-button resize-none"></textarea>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b-2 border-gray-300">
                                <th class="text-left py-3 px-4 font-bold text-lgu-headline">Event Type</th>
    ...
    <script>
        ...
        function initMap() {
            ...
            if (map) map.remove();
            map = L.map('reservationMap').setView([14.6760, 120.9626], 11);
