...
try {
    // Get total collected payments from residents (available budget)
    $stmt = $pdo->prepare("
    ...
    $total_budget = (float)($collected_row['total_collected'] ?? 0);
    
    // Get allocated budget (approved fund requests)
    $stmt = $pdo->prepare("
    ...
    <main class="flex-1 p-3 sm:p-4 lg:p-6 overflow-y-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        <div class="stat-card bg-white rounded-lg shadow p-4 border-l-4 border-lgu-button">
          ...
          <p class="text-2xl sm:text-3xl font-bold text-lgu-headline mt-2">₱<?php echo number_format($total_budget, 2); ?></p>
        </div>
        <div class="stat-card bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
          <p class="text-xs font-semibold text-lgu-paragraph uppercase">Allocated</p>
        ...
        <div class="stat-card bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
          <p class="text-xs font-semibold text-lgu-paragraph uppercase">Remaining</p>
          <p class="text-2xl sm:text-3xl font-bold text-green-600 mt-2">₱<?php echo number_format($remaining_budget, 2); ?></p>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const budgetData = {
    ...
    remaining: <?php echo json_encode($remaining_budget); ?>
  };
  ...
  if (barCtx) {
    new Chart(barCtx, {
      ...
      data: {
        ...
        datasets: [{
          label: 'Amount (₱)',
          data: [budgetData.total, budgetData.allocated, budgetData.spent, budgetData.remaining],
          backgroundColor: ['#faae2b', '#3b82f6', '#ef4444', '#10b981'],
          borderColor: ['#f59e0b', '#1d4ed8', '#dc2626', '#059669'],
          borderWidth: 1
        }]
      },
      options: {
