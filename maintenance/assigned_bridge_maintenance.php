...
// Fetch assigned BRIDGE maintenance tasks for this user
$query = "
    SELECT 
        ...
        COALESCE(fr.status, 'none') as budget_status,
        COALESCE(fr.id, 0) as budget_request_id
    ...
    LEFT JOIN fund_requests fr ON ma.report_id = fr.report_id AND ma.assigned_to = fr.maintenance_team_id
    WHERE ma.assigned_to = ? AND ma.team_type = 'bridge_maintenance'
    ORDER BY 
        CASE 
...
// Handle status updates for BRIDGE maintenance
    ...
    if (isset($_POST['start_work'])) {
        ...
        try {
            $update_query = "
                ...
                WHERE id = ? AND assigned_to = ? AND team_type = 'bridge_maintenance'
            ";
            $stmt = $pdo->prepare($update_query);
            $stmt->execute([$assignment_id, $user_id]);
            
            $_SESSION['success'] = "Bridge maintenance work started successfully!";
            header('Location: assigned_bridge_maintenance.php');
            exit();
    ...
    if (isset($_POST['complete_work'])) {
        ...
        try {
            ...
            if (isset($_FILES['completion_image']) && $_FILES['completion_image']['error'] === UPLOAD_ERR_OK) {
                ...
                $filename = 'completion_bridge_' . $assignment_id . '_' . time() . '.' . $file_extension;
                $target_path = $upload_dir . $filename;
                
                // Validate file type - allow both images and videos
                $allowed_types = ['jpg', 'jpeg', 'png', 'gif', 'webm', 'mp4', 'mov', 'avi'];
                if (in_array(strtolower($file_extension), $allowed_types)) {
                    if (move_uploaded_file($_FILES['completion_image']['tmp_name'], $target_path)) {
                        $completion_image_path = 'completion/' . $filename;
                    } else {
                        throw new Exception("Failed to upload completion media.");
                    }
                } else {
                    throw new Exception("Invalid file type. Only JPG, JPEG, PNG, GIF, WebM, MP4, MOV, and AVI are allowed.");
                }
            }
            ...
            $pdo->commit();
            $_SESSION['success'] = "Bridge maintenance work completed successfully!" . ($completion_image_path ? " Completion image uploaded." : "");
            header('Location: assigned_bridge_maintenance.php');
            exit();
...
<head>
  ...
  <title>Bridge Maintenance Tasks - RTIM</title>
...
<body class="bg-lgu-bg min-h-screen font-poppins">
    ...
    <header class="sticky top-0 z-40 bg-white shadow-sm border-b border-gray-200">
      <div class="flex items-center justify-between px-4 py-3 gap-4">
        ...
        <div class="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div class="min-w-0">
            <h1 class="text-xl sm:text-2xl font-bold text-lgu-headline truncate">Bridge Maintenance Tasks</h1>
            <p class="text-xs sm:text-sm text-lgu-paragraph truncate">Manage your assigned bridge maintenance tasks</p>
          </div>
        </div>
        ...
        <div class="stat-card bg-white rounded-lg shadow-sm p-4 border-l-4 border-lgu-button">
          <div class="flex justify-between items-center gap-3">
            ...
            <div class="w-12 h-12 bg-amber-50 rounded-lg flex items-center justify-center flex-shrink-0">
              <i class="fa fa-bridge text-lg sm:text-xl text-lgu-button"></i>
            </div>
          </div>
        </div>
        ...
        <div class="stat-card bg-white rounded-lg shadow-sm p-4 border-l-4 border-lgu-stroke">
          <div class="flex justify-between items-center gap-3">
            ...
            <div class="w-12 h-12 bg-lgu-bg rounded-lg flex items-center justify-center flex-shrink-0">
              <i class="fa fa-bridge text-lg sm:text-xl text-lgu-headline"></i>
            </div>
          </div>
        </div>
      </div>

      <!-- Bridge Maintenance Tasks List -->
      <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200">
        <div class="bg-lgu-headline text-white px-4 py-3 sm:px-6 flex items-center justify-between">
          <h2 class="text-lg sm:text-xl font-bold flex items-center">
            <i class="fas fa-bridge mr-2"></i>
            Bridge Maintenance Tasks
          </h2>
        ...
        <div class="p-4 sm:p-6">
          <?php if (empty($assignments)): ?>
            <div class="text-center py-8">
              <i class="fas fa-bridge fa-3x text-gray-300 mb-3"></i>
              <h3 class="text-lg font-semibold text-lgu-paragraph">No bridge maintenance tasks assigned to you yet.</h3>
              <p class="text-gray-400 mt-2">You will see assigned bridge maintenance tasks here once they are assigned to you by an administrator.</p>
            </div>
          <?php else: ?>
            <div class="space-y-4">
                  ...
                  <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    ...
                    <div class="flex-1">
                      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3">
                        ...
                        <div class="mt-2 sm:mt-0 flex items-center gap-2 flex-wrap">
                          ...
                          <?php if ($assignment['assignment_status'] !== 'completed'): ?>
                            <?php if ($assignment['budget_status'] === 'approved'): ?>
                            ...
                            <?php elseif ($assignment['budget_status'] === 'pending'): ?>
                            ...
                            <?php elseif ($assignment['budget_status'] === 'rejected'): ?>
                      ...
                      <div class="mt-4 bg-gray-50 rounded-lg p-3 border border-gray-200">
                        <div class="flex items-center gap-2 mb-2">
                          ...
                          <span class="font-medium text-lgu-headline text-sm">Bridge Location</span>
                        </div>
                        <div id="taskMap_<?= $assignment['assignment_id'] ?>" class="w-full rounded border border-gray-300"></div>
                        <p class="text-xs text-lgu-paragraph mt-1">Red marker shows the bridge maintenance location</p>
                      </div>
                    </div>
                    
                    <!-- Action Buttons & Image -->
                    <div class="flex flex-col items-center gap-4 lg:w-64">
                      <?php if (!empty($assignment['image_path'])): ?>
                      ...
                      <div class="flex flex-col gap-2 w-full">
                        <?php if ($assignment['assignment_status'] === 'assigned'): ?>
                          <?php $budget_approved = $assignment['budget_status'] === 'approved'; ?>
                          <form method="POST" class="w-full">
                            <input type="hidden" name="assignment_id" value="<?= $assignment['assignment_id'] ?>">
                            <button type="submit" name="start_work" class="w-full <?= $budget_approved ? 'bg-lgu-button hover:bg-amber-500' : 'bg-gray-400 cursor-not-allowed' ?> text-lgu-button-text font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition shadow-sm" <?= !$budget_approved ? 'disabled' : '' ?>>
                              <i class="fas fa-play-circle mr-2"></i> Start Bridge Work
                            </button>
                          </form>
                          <?php if (!$budget_approved): ?>
                            <div class="w-full bg-red-50 border border-red-200 rounded-lg p-3 text-center">
                              <p class="text-xs text-red-700 font-semibold mb-2">
                                <i class="fas fa-exclamation-circle mr-1"></i> Budget Request Required
                              </p>
                              <p class="text-xs text-red-600 mb-2">You must submit and get approval for a budget request before starting this work.</p>
                              <a href="fund_requests.php" class="inline-block bg-red-600 hover:bg-red-700 text-white font-semibold py-1 px-3 rounded text-xs transition">
                                <i class="fas fa-file-invoice-dollar mr-1"></i> Request Budget
                              </a>
                            </div>
                          <?php endif; ?>
                        <?php elseif ($assignment['assignment_status'] === 'in_progress'): ?>
                          <button class="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition shadow-sm complete-bridge-work" 
                                  data-assignment-id="<?= $assignment['assignment_id'] ?>">
                            <i class="fas fa-check-circle mr-2"></i> Complete Bridge Work
                          </button>
                        <?php elseif ($assignment['assignment_status'] === 'completed'): ?>
                          <div class="text-center w-full py-2 px-4 border border-green-300 rounded-lg bg-green-50">
                            <span class="text-green-700 font-semibold flex items-center justify-center">
                              <i class="fas fa-check-double mr-2"></i> Bridge Work Completed
                            </span>
                            <?php if ($assignment['completed_at']): ?>
                        ...
                        <!-- View Details Button -->
                        <button class="w-full bg-gray-200 hover:bg-gray-300 text-lgu-button-text font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition shadow-sm view-bridge-details"
  ...
  <!-- Complete Bridge Work Modal with Image Upload -->
  <div class="modal fade fixed inset-0 z-50 overflow-y-auto" id="completeBridgeModal" tabindex="-1" aria-labelledby="completeBridgeModalLabel" aria-hidden="true" style="display: none;">
      ...
      <div class="relative inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-lg rounded-lg border border-gray-200">
        <div class="bg-lgu-headline text-white px-6 py-4 rounded-t-lg">
          <h3 class="text-lg font-semibold flex items-center">
            <i class="fas fa-check-circle mr-2"></i> Complete Bridge Maintenance
          </h3>
        </div>
        
        <form method="POST" action="" enctype="multipart/form-data" class="mt-4">
          <input type="hidden" name="assignment_id" id="complete_bridge_assignment_id">
          
          <div class="mb-4">
            <label for="completion_notes" class="block text-sm font-medium text-lgu-headline mb-2">
              <i class="fas fa-sticky-note mr-1"></i> Bridge Work Completion Notes
            </label>
            <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-lgu-button focus:border-lgu-button" 
                      ...
                      placeholder="Describe the bridge maintenance work completed (structural repairs, painting, drainage work, etc.) and any follow-up required..." 
                      required></textarea>
            <div class="text-xs text-lgu-paragraph mt-1">Please provide details about the bridge maintenance work you completed.</div>
          </div>
          
          <div class="mb-4">
            <label class="block text-sm font-medium text-lgu-headline mb-2">
              <i class="fas fa-camera mr-1"></i> Media Evidence
            </label>
            <p class="text-lgu-paragraph text-sm mb-4">Add a photo or video to document the completed bridge work</p>
            
            <!-- Tabs for upload methods -->
            <div class="flex gap-1 mb-4 bg-gray-100 p-1 rounded-lg w-fit">
              <button type="button" class="completion-tab-btn active px-4 py-2 font-medium text-lgu-headline rounded-lg" data-tab="completion-camera">
                <i class="fas fa-camera mr-2"></i>Capture Photo
              </button>
              <button type="button" class="completion-tab-btn px-4 py-2 font-medium text-lgu-paragraph rounded-lg hover:text-lgu-headline" data-tab="completion-video">
                <i class="fas fa-video mr-2"></i>Capture Video
              </button>
            </div>
            
            <!-- Camera Tab -->
            <div id="completion-camera-tab" class="completion-tab-content bg-gray-50 rounded-lg p-4">
              <video id="completionCameraVideo" class="w-full max-w-xs rounded-lg border-2 border-gray-300 mb-4 bg-black" playsinline autoplay></video>
              <div class="flex gap-2 mb-4">
                <button type="button" id="startCompletionCameraBtn" class="flex-1 bg-lgu-button text-lgu-button-text font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition">
                  <i class="fas fa-play mr-2"></i>Start Camera
                </button>
                <button type="button" id="stopCompletionCameraBtn" class="flex-1 bg-red-600 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition hidden">
                  <i class="fas fa-stop mr-2"></i>Stop Camera
                </button>
                <button type="button" id="captureCompletionPhotoBtn" class="flex-1 bg-green-600 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition hidden">
                  <i class="fas fa-camera mr-2"></i>Capture
                </button>
              </div>
              <canvas id="completionCaptureCanvas" style="display: none;"></canvas>
              <img id="completionCapturedImage" class="w-full max-w-xs rounded-lg border-2 border-gray-300 hidden" alt="Captured photo">
              <div id="completionCaptureTimestamp" class="mt-3 p-2 bg-green-50 border border-green-200 rounded-lg hidden text-xs text-green-800">
                <i class="fas fa-clock mr-1"></i>Photo captured on: <span id="completionTimestampText"></span>
              </div>
            </div>
            
            <!-- Video Tab -->
            <div id="completion-video-tab" class="completion-tab-content hidden bg-gray-50 rounded-lg p-4">
              <video id="completionVideoPreview" class="w-full max-w-xs rounded-lg border-2 border-gray-300 mb-4 bg-black" playsinline autoplay muted></video>
              <div class="flex gap-2 mb-4">
                <button type="button" id="startCompletionVideoBtn" class="flex-1 bg-lgu-button text-lgu-button-text font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition">
                  <i class="fas fa-play mr-2"></i>Start Camera
                </button>
                <button type="button" id="recordCompletionVideoBtn" class="flex-1 bg-red-600 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition hidden">
                  <i class="fas fa-record-vinyl mr-2"></i>Record
                </button>
                <button type="button" id="stopCompletionVideoBtn" class="flex-1 bg-gray-600 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition hidden">
                  <i class="fas fa-stop mr-2"></i>Stop
                </button>
              </div>
              <div id="completionRecordingTimer" class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg hidden text-center text-red-800 font-bold">
                <i class="fas fa-circle text-red-600 mr-2 animate-pulse"></i>Recording: <span id="completionTimerDisplay">00:30</span>
              </div>
              <video id="completionRecordedVideo" class="w-full max-w-xs rounded-lg border-2 border-gray-300 hidden" controls playsinline></video>
              <div id="completionVideoTimestamp" class="mt-3 p-2 bg-green-50 border border-green-200 rounded-lg hidden text-xs text-green-800">
                <i class="fas fa-clock mr-1"></i>Video recorded on: <span id="completionVideoTimestampText"></span>
              </div>
            </div>
            
            <!-- Photo Actions -->
            <div id="completionPhotoActions" class="mt-3 hidden">
              <div class="flex gap-2">
                <button type="button" id="changeCompletionPhotoBtn" class="flex-1 bg-lgu-button text-lgu-button-text px-3 py-2 rounded-lg text-sm hover:bg-yellow-500 transition flex items-center justify-center">
                  <i class="fas fa-sync-alt mr-2"></i>Change Media
                </button>
                <button type="button" id="removeCompletionPhotoBtn" class="flex-1 bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600 transition flex items-center justify-center">
          ...
          <div class="flex justify-end space-x-3 mt-6">
            <button type="button" class="px-4 py-2 text-sm font-medium text-lgu-paragraph bg-gray-200 hover:bg-gray-300 rounded-lg transition cancel-bridge-complete">
            ...
            <button type="submit" name="complete_work" class="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-lg transition">
              <i class="fas fa-check mr-1"></i> Complete Bridge Work
            </button>
          </div>
        </form>
      </div>
  ...
  <!-- Bridge Task Details Modal -->
  <div class="modal fade fixed inset-0 z-50 overflow-y-auto" id="bridgeDetailsModal" tabindex="-1" aria-labelledby="bridgeDetailsModalLabel" aria-hidden="true" style="display: none;">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
      <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" aria-hidden="true"></div>
      ...
      <div class="relative inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-lg rounded-lg border border-gray-200">
        <div class="bg-lgu-headline text-white px-6 py-4 rounded-t-lg">
          <h3 class="text-lg font-semibold flex items-center">
            <i class="fas fa-info-circle mr-2"></i> Bridge Task Details
          </h3>
        </div>
        
        <div class="mt-4 space-y-4" id="bridge-task-details-content">
          <!-- Details will be populated by JavaScript -->
        </div>
        
        <div class="flex justify-end mt-6">
          <button type="button" class="px-4 py-2 text-sm font-medium text-lgu-paragraph bg-gray-200 hover:bg-gray-300 rounded-lg transition close-bridge-details">
  ...
  <div class="modal fade fixed inset-0 z-50 overflow-y-auto" id="imageModal" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
      ...
      <div class="relative inline-block w-full max-w-4xl p-4 my-8 text-center align-middle transition-all transform">
        ...
        <img id="modalImage" src="" alt="Bridge Hazard Image" class="max-w-full max-h-[80vh] rounded-lg shadow-lg mx-auto border-2 border-white">
      </div>
    </div>
  </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Complete bridge work modal functionality
  const completeBridgeModal = document.getElementById('completeBridgeModal');
  const completeBridgeButtons = document.querySelectorAll('.complete-bridge-work');
  
  completeBridgeButtons.forEach(button => {
    button.addEventListener('click', function() {
      const assignmentId = this.getAttribute('data-assignment-id');
      document.getElementById('complete_bridge_assignment_id').value = assignmentId;
      completeBridgeModal.style.display = 'block';
    });
  });
  
  // Close complete bridge modal
  document.querySelector('.cancel-bridge-complete').addEventListener('click', function() {
    completeBridgeModal.style.display = 'none';
    if (completionCameraStream) {
      completionCameraStream.getTracks().forEach(track => track.stop());
      completionCameraStream = null;
    }
    if (completionVideoStream) {
      completionVideoStream.getTracks().forEach(track => track.stop());
      completionVideoStream = null;
    }
  });
  
  // Bridge task details modal
  const bridgeDetailsModal = document.getElementById('bridgeDetailsModal');
  const viewBridgeDetailsButtons = document.querySelectorAll('.view-bridge-details');
  
  viewBridgeDetailsButtons.forEach(button => {
    button.addEventListener('click', function() {
      const assignmentData = JSON.parse(this.getAttribute('data-assignment'));
      populateBridgeDetailsModal(assignmentData);
      bridgeDetailsModal.style.display = 'block';
    });
  });
  
  // Close bridge details modal
  document.querySelector('.close-bridge-details').addEventListener('click', function() {
    bridgeDetailsModal.style.display = 'none';
  });
  
  // Close modals when clicking outside
  window.addEventListener('click', function(event) {
    if (event.target === completeBridgeModal) {
      completeBridgeModal.style.display = 'none';
      if (completionCameraStream) {
        completionCameraStream.getTracks().forEach(track => track.stop());
        completionCameraStream = null;
    ...
    if (event.target === bridgeDetailsModal) {
      bridgeDetailsModal.style.display = 'none';
    }
  ...
  // Completion media capture functionality
  let completionCameraStream = null;
  let completionVideoStream = null;
  let completionMediaRecorder = null;
  let completionRecordedChunks = [];
  const completionFileInput = document.createElement('input');
  completionFileInput.type = 'file';
  completionFileInput.name = 'completion_image';
  completionFileInput.accept = 'image/*,video/*';
  completionFileInput.style.display = 'none';
  document.getElementById('completeBridgeModal').appendChild(completionFileInput);
  
  // Tab switching
  document.querySelectorAll('.completion-tab-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      const tabName = this.getAttribute('data-tab');
      document.querySelectorAll('.completion-tab-btn').forEach(b => {
      ...
      document.querySelectorAll('.completion-tab-content').forEach(content => {
        content.classList.add('hidden');
      });
      document.getElementById(tabName + '-tab').classList.remove('hidden');
      if (tabName !== 'completion-camera' && completionCameraStream) {
        completionCameraStream.getTracks().forEach(track => track.stop());
        completionCameraStream = null;
      }
      if (tabName !== 'completion-video' && completionVideoStream) {
        if (completionMediaRecorder && completionMediaRecorder.state === 'recording') {
          completionMediaRecorder.stop();
        }
        completionVideoStream.getTracks().forEach(track => track.stop());
        completionVideoStream = null;
      }
  ...
  document.getElementById('startCompletionCameraBtn').addEventListener('click', async function() {
    try {
      completionCameraStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } });
      document.getElementById('completionCameraVideo').srcObject = completionCameraStream;
      this.classList.add('hidden');
      document.getElementById('stopCompletionCameraBtn').classList.remove('hidden');
      document.getElementById('captureCompletionPhotoBtn').classList.remove('hidden');
    } catch (err) {
      alert('Unable to access camera: ' + err.message);
    }
  });
  
  document.getElementById('stopCompletionCameraBtn').addEventListener('click', function() {
    if (completionCameraStream) {
      completionCameraStream.getTracks().forEach(track => track.stop());
      completionCameraStream = null;
    }
    document.getElementById('completionCameraVideo').srcObject = null;
    document.getElementById('startCompletionCameraBtn').classList.remove('hidden');
    this.classList.add('hidden');
    document.getElementById('captureCompletionPhotoBtn').classList.add('hidden');
  });
  
  document.getElementById('captureCompletionPhotoBtn').addEventListener('click', function() {
    const canvas = document.getElementById('completionCaptureCanvas');
    const video = document.getElementById('completionCameraVideo');
    const img = document.getElementById('completionCapturedImage');
    ...
    ctx.fillText(now, 20, canvas.height - 30);
    img.src = canvas.toDataURL('image/jpeg');
    img.classList.remove('hidden');
    document.getElementById('completionCaptureTimestamp').classList.remove('hidden');
    document.getElementById('completionTimestampText').textContent = now;
    canvas.toBlob(blob => {
      const file = new File([blob], 'completion_' + Date.now() + '.jpg', { type: 'image/jpeg' });
      const dt = new DataTransfer();
      dt.items.add(file);
      completionFileInput.files = dt.files;
      document.getElementById('completionPhotoActions').classList.remove('hidden');
    }, 'image/jpeg', 0.8);
  });
  
  // Video functionality
  document.getElementById('startCompletionVideoBtn').addEventListener('click', async function() {
    try {
      completionVideoStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' }, audio: true });
      document.getElementById('completionVideoPreview').srcObject = completionVideoStream;
      this.classList.add('hidden');
      document.getElementById('recordCompletionVideoBtn').classList.remove('hidden');
    } catch (err) {
      alert('Unable to access camera: ' + err.message);
    }
  });
  
  let completionRecordingTimer = null;
  let completionRecordingStartTime = null;
  
  document.getElementById('recordCompletionVideoBtn').addEventListener('click', function() {
    completionRecordedChunks = [];
    const now = new Date().toLocaleString();
    window.completionVideoTimestamp = now;
    ...
    completionMediaRecorder = new MediaRecorder(completionVideoStream, options);
    completionMediaRecorder.ondataavailable = (e) => { if (e.data.size > 0) completionRecordedChunks.push(e.data); };
    completionMediaRecorder.onstop = () => {
      const mimeType = completionMediaRecorder.mimeType || 'video/webm';
      const ext = mimeType.includes('mp4') ? 'mp4' : 'webm';
      const blob = new Blob(completionRecordedChunks, { type: mimeType });
      const file = new File([blob], 'completion_' + Date.now() + '.' + ext, { type: mimeType });
      const dt = new DataTransfer();
      dt.items.add(file);
      completionFileInput.files = dt.files;
      document.getElementById('completionRecordedVideo').src = URL.createObjectURL(blob);
      document.getElementById('completionRecordedVideo').classList.remove('hidden');
      document.getElementById('completionVideoTimestamp').classList.remove('hidden');
      document.getElementById('completionVideoTimestampText').textContent = window.completionVideoTimestamp;
      document.getElementById('completionPhotoActions').classList.remove('hidden');
      if (completionRecordingTimer) clearInterval(completionRecordingTimer);
      document.getElementById('completionRecordingTimer').classList.add('hidden');
    };
    completionMediaRecorder.start();
    this.classList.add('hidden');
    document.getElementById('stopCompletionVideoBtn').classList.remove('hidden');
    document.getElementById('completionRecordingTimer').classList.remove('hidden');
    completionRecordingStartTime = Date.now();
    completionRecordingTimer = setInterval(() => {
      const elapsed = Math.floor((Date.now() - completionRecordingStartTime) / 1000);
      const remaining = Math.max(0, 30 - elapsed);
      const m = Math.floor(remaining / 60);
      const s = remaining % 60;
      document.getElementById('completionTimerDisplay').textContent = m.toString().padStart(2, '0') + ':' + s.toString().padStart(2, '0');
      if (remaining === 0) document.getElementById('stopCompletionVideoBtn').click();
    }, 1000);
  });
  
  document.getElementById('stopCompletionVideoBtn').addEventListener('click', function() {
    if (completionMediaRecorder && completionMediaRecorder.state === 'recording') {
      completionMediaRecorder.stop();
    }
    if (completionRecordingTimer) clearInterval(completionRecordingTimer);
  ...
  document.getElementById('changeCompletionPhotoBtn').addEventListener('click', function() {
    completionFileInput.value = '';
    document.getElementById('completionCapturedImage').classList.add('hidden');
    document.getElementById('completionRecordedVideo').classList.add('hidden');
    document.getElementById('completionCaptureTimestamp').classList.add('hidden');
    document.getElementById('completionVideoTimestamp').classList.add('hidden');
    document.getElementById('completionPhotoActions').classList.add('hidden');
  });
  
  document.getElementById('removeCompletionPhotoBtn').addEventListener('click', function() {
    document.getElementById('changeCompletionPhotoBtn').click();
  });
  
  // Function to populate bridge details modal
  function populateBridgeDetailsModal(assignment) {
    const content = document.getElementById('bridge-task-details-content');
    ...
    content.innerHTML = `
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <h4 class="font-semibold text-lgu-headline">Bridge Task Information</h4>
    ...
    if (data.results && data.results.length > 0) {
      ...
      const marker = L.marker([lat, lng], {
        icon: L.divIcon({
          ...
          html: '<div style="background: #ef4444; width: 20px; height: 20px; border-radius: 50%; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center;"><i class="fas fa-bridge text-white text-xs"></i></div>',
          iconSize: [20, 20],
          iconAnchor: [10, 10]
        })
      }).addTo(map);
      ...
      let popupContent = `
        <div class="p-2 text-xs">
          <h4 class="font-bold text-red-600 mb-1">Bridge Maintenance</h4>
