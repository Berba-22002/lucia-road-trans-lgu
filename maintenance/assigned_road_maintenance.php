...
// Fetch assigned ROAD maintenance tasks for this user
$query = "
    SELECT 
    ...
    WHERE ma.assigned_to = ? AND ma.team_type = 'road_maintenance'
    ORDER BY 
        CASE 
...
// Handle status updates for ROAD maintenance
    ...
    if (isset($_POST['start_work'])) {
        ...
        try {
            $update_query = "
                ...
                WHERE id = ? AND assigned_to = ? AND team_type = 'road_maintenance'
            ";
            $stmt = $pdo->prepare($update_query);
            $stmt->execute([$assignment_id, $user_id]);
            
            $_SESSION['success'] = "Road maintenance work started successfully!";
            header('Location: assigned_road_maintenance.php');
            exit();
    ...
    if (isset($_POST['complete_work'])) {
        ...
        try {
            ...
            if (isset($_FILES['completion_image']) && $_FILES['completion_image']['error'] === UPLOAD_ERR_OK) {
                ...
                $filename = 'completion_' . $assignment_id . '_' . time() . '.' . $file_extension;
            ...
            $pdo->commit();
            $_SESSION['success'] = "Road maintenance work completed successfully!" . ($completion_image_path ? " Completion image uploaded." : "");
            header('Location: assigned_road_maintenance.php');
            exit();
...
<head>
  ...
  <title>Road Maintenance Tasks - RTIM</title>
...
<body class="bg-lgu-bg min-h-screen font-poppins">
    ...
    <header class="sticky top-0 z-40 bg-white shadow-sm border-b border-gray-200">
      <div class="flex items-center justify-between px-4 py-3 gap-4">
        ...
        <div class="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div class="min-w-0">
            <h1 class="text-xl sm:text-2xl font-bold text-lgu-headline truncate">Road Maintenance Tasks</h1>
            <p class="text-xs sm:text-sm text-lgu-paragraph truncate">Manage your assigned road maintenance tasks</p>
          </div>
        </div>
        ...
        <div class="stat-card bg-white rounded-lg shadow-sm p-4 border-l-4 border-lgu-stroke">
          <div class="flex justify-between items-center gap-3">
            ...
            <div class="w-12 h-12 bg-lgu-bg rounded-lg flex items-center justify-center flex-shrink-0">
              <i class="fa fa-road text-lg sm:text-xl text-lgu-headline"></i>
            </div>
          </div>
        </div>
      </div>

      <!-- Road Maintenance Tasks List -->
        ...
        <div class="p-4 sm:p-6">
          <?php if (empty($assignments)): ?>
            <div class="text-center py-8">
              ...
              <h3 class="text-lg font-semibold text-lgu-paragraph">No road maintenance tasks assigned to you yet.</h3>
              <p class="text-gray-400 mt-2">You will see assigned road maintenance tasks here once they are assigned to you by an administrator.</p>
            </div>
          <?php else: ?>
            <div class="space-y-4">
                  ...
                  <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    ...
                    <div class="flex-1">
                      ...
                      <div class="mt-4 bg-gray-50 rounded-lg p-3 border border-gray-200">
                        <div class="flex items-center gap-2 mb-2">
                          ...
                          <span class="font-medium text-lgu-headline text-sm">Task Location</span>
                        </div>
                        <div id="taskMap_<?= $assignment['assignment_id'] ?>" class="w-full rounded border border-gray-300"></div>
                        <p class="text-xs text-lgu-paragraph mt-1">Red marker shows the maintenance location</p>
                      </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center gap-4 lg:w-64">
                      <?php if (!empty($assignment['image_path'])): ?>
                        <div class="text-center">
                          ...
                          if (substr($image_src, 0, 4) !== 'http' && substr($image_src, 0, 1) !== '/') {
                              // If it's a relative path, add the uploads directory prefix
                      ...
                      <div class="flex flex-col gap-2 w-full">
                        <?php if ($assignment['assignment_status'] === 'assigned'): ?>
                          ...
                          <form method="POST" class="w-full">
                            ...
                            <button type="submit" name="start_work" class="w-full <?= $budget_approved ? 'bg-lgu-button hover:bg-amber-500' : 'bg-gray-400 cursor-not-allowed' ?> text-lgu-button-text font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition shadow-sm" <?= !$budget_approved ? 'disabled' : '' ?>>
                              <i class="fas fa-play-circle mr-2"></i> Start Road Work
                            </button>
                          </form>
                          ...
                          <?php endif; ?>
                        <?php elseif ($assignment['assignment_status'] === 'in_progress'): ?>
                          <button class="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition shadow-sm complete-road-work" 
                                  data-assignment-id="<?= $assignment['assignment_id'] ?>">
                            <i class="fas fa-check-circle mr-2"></i> Complete Road Work
                          </button>
                        <?php elseif ($assignment['assignment_status'] === 'completed'): ?>
                          <div class="text-center w-full py-2 px-4 border border-green-300 rounded-lg bg-green-50">
                            <span class="text-green-700 font-semibold flex items-center justify-center">
                              <i class="fas fa-check-double mr-2"></i> Road Work Completed
                            </span>
                            <?php if ($assignment['completed_at']): ?>
                        ...
                        <!-- View Details Button -->
                        <button class="w-full bg-gray-200 hover:bg-gray-300 text-lgu-button-text font-semibold py-2 px-4 rounded-lg flex items-center justify-center transition shadow-sm view-road-details"
  ...
  <!-- Complete Road Work Modal with Image Upload -->
  <div class="modal fade fixed inset-0 z-50 overflow-y-auto" id="completeRoadModal" tabindex="-1" aria-labelledby="completeRoadModalLabel" aria-hidden="true" style="display: none;">
      ...
      <div class="relative inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-lg rounded-lg border border-gray-200">
        <div class="bg-lgu-headline text-white px-6 py-4 rounded-t-lg">
          <h3 class="text-lg font-semibold flex items-center">
            <i class="fas fa-check-circle mr-2"></i> Complete Road Maintenance
          </h3>
        </div>
        
        <form method="POST" action="" enctype="multipart/form-data" class="mt-4">
          <input type="hidden" name="assignment_id" id="complete_road_assignment_id">
          
          <div class="mb-4">
            <label for="completion_notes" class="block text-sm font-medium text-lgu-headline mb-2">
              <i class="fas fa-sticky-note mr-1"></i> Road Work Completion Notes
            </label>
            <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-lgu-button focus:border-lgu-button" 
                      ...
                      placeholder="Describe the road maintenance work completed, materials used (asphalt, concrete, etc.), and any follow-up required..." 
                      required></textarea>
            <div class="text-xs text-lgu-paragraph mt-1">Please provide details about the road repair work you completed.</div>
          </div>
          ...
          <div class="flex justify-end space-x-3 mt-6">
            <button type="button" class="px-4 py-2 text-sm font-medium text-lgu-paragraph bg-gray-200 hover:bg-gray-300 rounded-lg transition cancel-road-complete">
            ...
            <button type="submit" name="complete_work" class="px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-lg transition">
              <i class="fas fa-check mr-1"></i> Complete Road Work
            </button>
          </div>
        </form>
      </div>
  ...
  <!-- Road Task Details Modal -->
  <div class="modal fade fixed inset-0 z-50 overflow-y-auto" id="roadDetailsModal" tabindex="-1" aria-labelledby="roadDetailsModalLabel" aria-hidden="true" style="display: none;">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
      <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" aria-hidden="true"></div>
      ...
      <div class="relative inline-block w-full max-w-2xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-lg rounded-lg border border-gray-200">
        <div class="bg-lgu-headline text-white px-6 py-4 rounded-t-lg">
          <h3 class="text-lg font-semibold flex items-center">
            <i class="fas fa-info-circle mr-2"></i> Road Task Details
          </h3>
        </div>
        
        <div class="mt-4 space-y-4" id="road-task-details-content">
          <!-- Details will be populated by JavaScript -->
        </div>
        
        <div class="flex justify-end mt-6">
          <button type="button" class="px-4 py-2 text-sm font-medium text-lgu-paragraph bg-gray-200 hover:bg-gray-300 rounded-lg transition close-road-details">
  ...
  <div class="modal fade fixed inset-0 z-50 overflow-y-auto" id="imageModal" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
      ...
      <div class="relative inline-block w-full max-w-4xl p-4 my-8 text-center align-middle transition-all transform">
        ...
        <img id="modalImage" src="" alt="Road Hazard Image" class="max-w-full max-h-[80vh] rounded-lg shadow-lg mx-auto border-2 border-white">
      </div>
    </div>
  </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Complete road work modal functionality
  const completeRoadModal = document.getElementById('completeRoadModal');
  const completeRoadButtons = document.querySelectorAll('.complete-road-work');
  
  completeRoadButtons.forEach(button => {
    button.addEventListener('click', function() {
      const assignmentId = this.getAttribute('data-assignment-id');
      document.getElementById('complete_road_assignment_id').value = assignmentId;
      completeRoadModal.style.display = 'block';
    });
  });
  
  // Close complete road modal
  document.querySelector('.cancel-road-complete').addEventListener('click', function() {
    completeRoadModal.style.display = 'none';
    clearImage();
  });
  
  // Road task details modal
  const roadDetailsModal = document.getElementById('roadDetailsModal');
  const viewRoadDetailsButtons = document.querySelectorAll('.view-road-details');
  
  viewRoadDetailsButtons.forEach(button => {
    button.addEventListener('click', function() {
      const assignmentData = JSON.parse(this.getAttribute('data-assignment'));
      populateRoadDetailsModal(assignmentData);
      roadDetailsModal.style.display = 'block';
    });
  });
  
  // Close road details modal
  document.querySelector('.close-road-details').addEventListener('click', function() {
    roadDetailsModal.style.display = 'none';
  });
  
  // Close modals when clicking outside
  window.addEventListener('click', function(event) {
    if (event.target === completeRoadModal) {
      completeRoadModal.style.display = 'none';
      clearImage();
    }
    if (event.target === roadDetailsModal) {
      roadDetailsModal.style.display = 'none';
    }
  ...
  // Function to populate road details modal
  function populateRoadDetailsModal(assignment) {
    const content = document.getElementById('road-task-details-content');
    ...
    content.innerHTML = `
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <h4 class="font-semibold text-lgu-headline">Road Task Information</h4>
    ...
    if (data.results && data.results.length > 0) {
      ...
      const marker = L.marker([lat, lng], {
        icon: L.divIcon({
          ...
          html: '<div style="background: #ef4444; width: 20px; height: 20px; border-radius: 50%; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center;"><i class="fas fa-tools text-white text-xs"></i></div>',
          iconSize: [20, 20],
          iconAnchor: [10, 10]
        })
      }).addTo(map);
      ...
      let popupContent = `
        <div class="p-2 text-xs">
          <h4 class="font-bold text-red-600 mb-1">Maintenance Task</h4>
