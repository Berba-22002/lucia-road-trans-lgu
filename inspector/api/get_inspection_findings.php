...
try {
    ...
    // Fetch inspection findings for the report - specifically filter for minor severity
    $sql = "
        SELECT 
            ri.id,
            ri.report_id,
            ri.inspector_id,
            ri.severity_level as severity,
            ri.notes,
            ri.assigned_at as created_at,
            u.fullname as inspector_name,
            DATE_FORMAT(ri.assigned_at, '%M %d, %Y at %h:%i %p') as formatted_date
        FROM report_inspectors ri
        LEFT JOIN users u ON ri.inspector_id = u.id
        WHERE ri.report_id = ? AND ri.severity_level = 'minor'
    ...
    echo json_encode([
        'success' => false, 
        'error' => 'Database error: Failed to fetch report inspector data',
