...
// Fetch road maintenance assignments
$query = "
    SELECT 
        ma.id,
        ma.report_id,
...
$stmt->execute();
$road_assignments = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
    ...
    <title>Maintenance Planner</title>
            ...
            <header class="bg-white shadow-sm border-b border-lgu-stroke">
                <div class="flex items-center justify-between p-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <h1 class="text-xl font-semibold text-lgu-headline">Maintenance Planner</h1>
                    </div>
                </div>
            </header>
                    ...
                    <div class="bg-lgu-headline px-6 py-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="bg-lgu-highlight text-lgu-button-text px-3 py-1 rounded-full text-sm font-medium">
                                <?= count($road_assignments) ?>
                            </span>
                        </div>
                    </div>
                    <div class="p-6">
                        <?php if (empty($road_assignments)): ?>
                            <div class="text-center py-8">
                                <div class="bg-lgu-bg rounded-xl p-6 max-w-md mx-auto border border-lgu-stroke">
                                    ...
                                    <p class="text-lgu-paragraph">No road maintenance assignments have been created yet.</p>
                                </div>
                            </div>
                        <?php else: ?>
                            <div class="space-y-4">
                                <?php foreach ($road_assignments as $assignment): ?>
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center space-x-4">
                                                <div class="bg-lgu-highlight bg-opacity-20 p-3 rounded-lg">
                                                    <i class="fas fa-road text-lgu-headline"></i>
                                                </div>
                                                <div>
