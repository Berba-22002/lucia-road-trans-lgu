...
// Fetch forwarded BRIDGE maintenance reports with detailed information
$query = "
    SELECT 
        rf.id as forward_id,
        r.id as report_id,
    ...
    WHERE rf.team_type = 'bridge_maintenance' 
...
// Fetch all bridge reports for table with pagination
...
// Build WHERE clause
$where = "r.hazard_type = 'bridge'";
if (!empty($search)) {
    $where .= " AND (r.id LIKE ? OR u.fullname LIKE ?)";
}
...
} elseif ($sort === 'reporter') {
    $order_by = "u.fullname ASC";
}

// Get total count
$count_query = "SELECT COUNT(*) FROM reports r LEFT JOIN users u ON r.user_id = u.id WHERE $where";
...
$params[] = $reports_limit;
$params[] = $reports_offset;
$all_reports_stmt->execute($params);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_maintenance'])) {
    ...
    try {
        ...
        $insert_assignment = "
            ...
            VALUES (?, ?, ?, ?, ?, ?, 'assigned', 'bridge_maintenance', NOW())
        ";
        ...
        if ($report_data) {
            ...
            $notification_stmt->execute([
                $assigned_to,
                'New Bridge Maintenance Assignment',
                'You have been assigned bridge maintenance work for report #' . $report_id . ' (' . ucfirst($report_data['hazard_type']) . ') at ' . $report_data['address'],
                'info'
            ]);
        }
        
        $pdo->commit();
        $_SESSION['success'] = "Bridge maintenance work assigned successfully!";
        header('Location: assign_bridge_team.php');
        exit();
...
<head>
    ...
    <title>Assign Bridge Maintenance</title>
...
<body class="bg-lgu-bg font-poppins">
    ...
    <div class="lg:ml-64 flex flex-col h-screen">
                ...
                <div class="flex items-center justify-between p-4">
                    <div class="flex items-center space-x-3">
                        <div class="p-2 bg-lgu-highlight rounded-lg">
                            <i class="fas fa-bridge text-lgu-button-text text-lg"></i>
                        </div>
                        <h1 class="text-xl font-semibold text-lgu-headline">Assign Bridge Maintenance</h1>
                    </div>
                </div>
            </header>
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <!-- Pending Bridge Maintenance Reports -->
                                ...
                                <?php if (empty($reports)): ?>
                                    <div class="text-center py-8">
                                        <div class="bg-lgu-bg rounded-xl p-6 max-w-md mx-auto border border-lgu-stroke">
                                            ...
                                            <p class="text-lgu-paragraph">All bridge maintenance reports have been assigned or there are no pending requests.</p>
                                        </div>
                                    </div>
                                <?php else: ?>
                                    <div class="space-y-4">
                                                    ...
                                                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                                        <div class="flex-1">
                                                            <div class="flex items-center gap-3 mb-3">
                                                                ...
                                                                <span class="bg-lgu-highlight bg-opacity-20 text-lgu-button-text px-2 py-1 rounded-full text-xs font-medium border border-lgu-highlight">
                                                                    Pending Assignment
                                                                </span>
                                                            </div>
                                                            ...
                                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                                                                <div class="flex items-start gap-2">
                                                                    <i class="fas fa-exclamation-triangle text-lgu-headline mt-1"></i>
                                                                    <div>
                                                                        <span class="font-medium text-lgu-paragraph">Hazard Type:</span>
                                                                        <p class="text-lgu-headline"><?= htmlspecialchars(ucfirst(str_replace('_', ' ', $report['hazard_type']))) ?></p>
                                                                    </div>
                                                                </div>
                                                                
                                                                <div class="flex items-start gap-2">
                                                                    <i class="fas fa-map-marker-alt text-lgu-headline mt-1"></i>
                                                                    <div>
                                                                        <span class="font-medium text-lgu-paragraph">Location:</span>
                                                                        <p class="text-lgu-headline"><?= htmlspecialchars($report['address']) ?></p>
                                                            ...
                                                            <div class="mt-3 flex items-start gap-2">
                                                                <i class="fas fa-phone text-lgu-paragraph mt-1"></i>
                                                                <div>
                                                                    <span class="font-medium text-lgu-paragraph">Contact Number:</span>
                                                                    <p class="text-lgu-headline"><?= htmlspecialchars($report['contact_number'] ?? 'N/A') ?></p>
                                                                </div>
                                                            </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-lgu-stroke">
                            ...
                            <div class="p-6">
                                <?php if (empty($maintenance_staff)): ?>
                                    <div class="bg-lgu-highlight bg-opacity-20 border border-lgu-highlight rounded-lg p-4 text-center">
                                        <i class="fas fa-exclamation-triangle text-lgu-button-text text-xl mb-2"></i>
                                        <p class="text-lgu-button-text font-medium">No maintenance staff available</p>
                                    </div>
                                <?php else: ?>
                                    <div class="space-y-3">
                                        <?php foreach ($maintenance_staff as $staff): ?>
                                            <div class="border border-lgu-stroke rounded-lg p-4 hover:shadow-sm transition-shadow duration-200 bg-white">
                                                <div class="flex items-start justify-between mb-2">
                                                    <h4 class="font-semibold text-lgu-headline"><?= htmlspecialchars($staff['fullname']) ?></h4>
                                                    <span class="bg-lgu-highlight bg-opacity-20 text-lgu-button-text px-2 py-1 rounded-full text-xs font-medium border border-lgu-highlight">
                                                        Available
                                                    </span>
                                                </div>
                                                <div class="space-y-1 text-sm text-lgu-paragraph">
                ...
                <!-- All Bridge Reports Table -->
                <div class="mt-6">
                    <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <input type="text" id="searchInput" placeholder="Search by ID or Reporter" class="px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-lgu-button" value="<?= htmlspecialchars($search) ?>">
                            ...
                            <select id="sortFilter" class="px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-lgu-button">
                                ...
                                <option value="reporter" <?= $sort === 'reporter' ? 'selected' : '' ?>>By Reporter</option>
                            </select>
                                        ...
                                        <tr class="hover:bg-gray-50 transition">
                                            ...
                                            <td class="px-4 py-3">
                                                <span class="inline-block bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold">
                                                    ...
                                                    <?= ucfirst(str_replace('_', ' ', $report['hazard_type'])) ?>
                                                </span>
                                            </td>
                                            ...
                                            <td class="px-4 py-3">
                                                ...
                                                $status_class = $status_map[$report['status']] ?? 'bg-gray-100 text-gray-800';
                                                ?>
                                            ...
                                            <td class="px-4 py-3 text-center">
                                                <?php if ($report['assigned_staff']): ?>
                                                    <span class="inline-block bg-lgu-highlight bg-opacity-20 text-lgu-button-text px-3 py-1 rounded-full text-xs font-semibold border border-lgu-highlight">
                                                        <?= htmlspecialchars($report['assigned_staff']) ?>
                                                    </span>
                                                <?php elseif ($report['ipm_id']): ?>
                                                    <span class="inline-block bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold">
                                ...
                                <div class="flex items-center gap-2 flex-wrap justify-center">
                                    <?php if ($total_reports_pages > 1): ?>
                                        <?php if ($reports_page > 1): ?>
                                            <a href="?reports_page=1&search=<?= urlencode($search) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>" class="px-3 py-1 rounded border border-gray-300 text-sm hover:bg-gray-200 transition">First</a>
                                            <a href="?reports_page=<?= $reports_page - 1 ?>&search=<?= urlencode($search) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>" class="px-3 py-1 rounded border border-gray-300 text-sm hover:bg-gray-200 transition">Prev</a>
                                        <?php endif; ?>
                                        ...
                                        <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                                            ...
                                            <?php else: ?>
                                                <a href="?reports_page=<?= $i ?>&search=<?= urlencode($search) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>" class="px-3 py-1 rounded border border-gray-300 text-sm hover:bg-gray-200 transition"><?= $i ?></a>
                                            <?php endif; ?>
                                        <?php endfor; ?>
                                        ...
                                        <?php if ($reports_page < $total_reports_pages): ?>
                                            <a href="?reports_page=<?= $reports_page + 1 ?>&search=<?= urlencode($search) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>" class="px-3 py-1 rounded border border-gray-300 text-sm hover:bg-gray-200 transition">Next</a>
                                            <a href="?reports_page=<?= $total_reports_pages ?>&search=<?= urlencode($search) ?>&status=<?= $status_filter ?>&sort=<?= $sort ?>" class="px-3 py-1 rounded border border-gray-300 text-sm hover:bg-gray-200 transition">Last</a>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </div>
                            </div>
    ...
    <div id="assignModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 hidden">
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-lgu-stroke">
            <div class="bg-lgu-headline px-6 py-4 rounded-t-xl">
                <div class="flex items-center justify-between">
                    <h3 class="text-white text-lg font-semibold flex items-center">
                        <i class="fas fa-user-plus mr-2"></i>
                        Assign Bridge Maintenance
                    </h3>
            ...
            <form method="POST" action="" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="assignment_notes" name="assignment_notes" rows="4"
                              placeholder="Add specific instructions for bridge maintenance work..."
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <button type="submit" name="assign_maintenance"
                        ...
                        <i class="fas fa-check"></i> Assign Bridge Work
                    </button>
                </div>
            </form>
        </div>
    ...
    <div id="imageModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50 hidden">
        <div class="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-lgu-stroke">
            <div class="flex justify-between items-center p-4 border-b border-lgu-stroke">
                <h3 class="text-lg font-semibold text-lgu-headline">Bridge Hazard Image</h3>
                ...
                if (data.results && data.results.length > 0) {
                    ...
                    let popupContent = `
                        <div class="p-2 text-xs">
                            <h4 class="font-bold text-red-600 mb-1">Bridge Hazard</h4>
