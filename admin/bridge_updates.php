...
$query = "
    ...
    WHERE ma.team_type = 'bridge_maintenance'
";
...
<head>
  ...
  <title>Bridge Maintenance Progress - RTIM Admin</title>
    ...
    <header class="sticky top-0 z-40 bg-white shadow-sm border-b border-gray-200">
      <div class="flex items-center justify-between px-4 py-3 gap-4">
        ...
        <div class="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div class="min-w-0">
            <h1 class="text-xl sm:text-2xl font-bold text-lgu-headline truncate">Bridge Maintenance Progress</h1>
            <p class="text-xs sm:text-sm text-lgu-paragraph truncate">Monitor all bridge maintenance assignments</p>
          </div>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-sm p-4 mb-6 border border-gray-200">
        <form method="GET" class="flex flex-col md:flex-row gap-3">
          ...
          <div class="flex items-end gap-2">
            ...
            <a href="progress_bridge_maintenance.php" class="bg-gray-200 hover:bg-gray-300 text-lgu-button-text font-semibold py-2 px-4 rounded-lg transition">
        ...
        <div class="p-4 sm:p-6">
          <?php if (empty($assignments)): ?>
            <div class="text-center py-8">
              ...
              <h3 class="text-lg font-semibold text-lgu-paragraph">No bridge maintenance tasks found</h3>
              <p class="text-gray-400 mt-2">
                <?php if (!empty($search) || $status_filter !== 'all'): ?>
                  Try adjusting your filters or search criteria.
                <?php else: ?>
                  Bridge maintenance tasks will appear here once assigned.
                <?php endif; ?>
              </p>
            </div>
          <?php else: ?>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
      ...
      <div class="relative inline-block w-full max-w-5xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-lg rounded-lg border border-gray-200">
        <div class="bg-lgu-headline text-white px-6 py-4 rounded-t-lg flex items-center justify-between">
          <h3 class="text-lg font-semibold flex items-center">
            <i class="fas fa-map mr-2"></i> Bridge Location Map
          </h3>
        ...
        <div class="mt-4">
          ...
          <div id="bridgeMap" class="w-full h-96 rounded-lg border-2 border-gray-300"></div>
          <p class="text-xs text-lgu-paragraph mt-2">Red marker shows the bridge maintenance location</p>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function populateDetailsModal(assignment) {
    ...
    if (assignment.assignment_status === 'assigned') {
      ...
      statusDescription = 'This bridge maintenance task has been assigned but the worker has not started working on it yet.';
      ...
      statusDescription = 'The worker has started working on this bridge maintenance task.';
      ...
      statusDescription = 'This bridge maintenance task has been successfully completed.';
    }
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="md:col-span-2">
          <div class="bg-lgu-bg p-4 rounded-lg border border-gray-200">
            ...
            ${isOverdue ? '<p class="text-sm text-red-600 font-semibold mt-2"><i class="fas fa-exclamation-triangle mr-1"></i>This bridge maintenance task is overdue!</p>' : ''}
          </div>
        </div>
        ...
        <div class="md:col-span-2">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="text-center">
              <img src="../uploads/hazard_reports/${assignment.image_path}" 
                   ...
                   onclick="openImageModal(this.src, 'Original Bridge Report Image')">
              <p class="text-xs text-lgu-paragraph mt-2">Original Bridge Report Image</p>
            </div>
            ` : ''}
            ...
            <div class="text-center">
              <img src="../uploads/${assignment.completion_image_path}" 
                   ...
                   onclick="openImageModal(this.src, 'Bridge Completion Photo')">
              <p class="text-xs text-green-600 font-semibold mt-2">
                <i class="fas fa-check-circle mr-1"></i>Bridge Completion Photo
              </p>
            </div>
            ` : ''}
          </div>
          <div class="mt-4">
            <button class="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition flex items-center justify-center"
              ...
              <i class="fas fa-map mr-2"></i>View Bridge Location on Map
            </button>
          </div>
        </div>
        ` : ''}
...
function openMapModal(address, landmark) {
  ...
  setTimeout(() => {
    initBridgeMap(address, landmark);
  }, 100);
}
...
function initBridgeMap(address, landmark) {
  // Initialize map centered on Philippines
  bridgeMap = L.map('bridgeMap').setView([14.5995, 120.9842], 12);
  ...
  if (address) {
    geocodeBridgeLocation(address, landmark);
  }
}

async function geocodeBridgeLocation(address, landmark) {
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      // Add bridge maintenance marker
      const marker = L.marker([lat, lng], {
        icon: L.divIcon({
          className: 'custom-marker',
          html: '<div style="background: #ef4444; width: 30px; height: 30px; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center;"><i class="fas fa-bridge text-white text-sm"></i></div>',
          iconSize: [30, 30],
          iconAnchor: [15, 15]
        })
      }).addTo(bridgeMap);
      ...
      const popupContent = `
        <div class="p-3 min-w-[200px]">
          <h4 class="font-bold text-red-600 mb-2">Bridge Maintenance Location</h4>
